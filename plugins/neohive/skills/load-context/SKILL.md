---
name: load-context
description: Use when the user says "start", "load context", "pre-load memory", "what do we know about X", or at the beginning of any session before doing real work. Calls `memory_context` with the current task description so directives, conventions, and relevant prior knowledge land in context before Codex starts editing or exploring.
---
